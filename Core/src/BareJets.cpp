...
void BareJets::init(){
    ...
    for(const auto& dR : dRToProduce)
    {
        BareFunctions::New(qglMult_dR[Form("0p%.0f",dR*1000)]);
        BareFunctions::New(qglPtD_dR[Form("0p%.0f",dR*1000)]);
        BareFunctions::New(qglAxis2_dR[Form("0p%.0f",dR*1000)]);
        BareFunctions::New(qglAxis1_dR[Form("0p%.0f",dR*1000)]);
        BareFunctions::New(qglGenMult_dR[Form("0p%.0f",dR*1000)]);
        BareFunctions::New(qglGenPtD_dR[Form("0p%.0f",dR*1000)]);
        BareFunctions::New(qglGenAxis2_dR[Form("0p%.0f",dR*1000)]);
        BareFunctions::New(qglGenAxis1_dR[Form("0p%.0f",dR*1000)]);
    }
...
void BareJets::clear(){
    ...
    for(const auto& dR : dRToProduce)
    {
        qglMult_dR[Form("0p%.0f",dR*1000)]	    ->clear();
        qglPtD_dR[Form("0p%.0f",dR*1000)]	    ->clear();
        qglAxis2_dR[Form("0p%.0f",dR*1000)]	    ->clear();
        qglAxis1_dR[Form("0p%.0f",dR*1000)]	    ->clear();
        qglGenMult_dR[Form("0p%.0f",dR*1000)]	->clear();
        qglGenPtD_dR[Form("0p%.0f",dR*1000)]	->clear();
        qglGenAxis2_dR[Form("0p%.0f",dR*1000)]	->clear();
        qglGenAxis1_dR[Form("0p%.0f",dR*1000)]	->clear();
    }
}
...
void BareJets::defineBranches(TTree *t){
    ...
    for(const auto& dR : dRToProduce)
    {
        t->Branch(jetName+Form("QglMult_dR0p%.0f",dR*1000),"vector<float>",&qglMult_dR[Form("0p%.0f",dR*1000)]	    );
        t->Branch(jetName+Form("QglPtD_dR0p%.0f",dR*1000),"vector<float>",&qglPtD_dR[Form("0p%.0f",dR*1000)]	    );
        t->Branch(jetName+Form("QglAxis2_dR0p%.0f",dR*1000),"vector<float>",&qglAxis2_dR[Form("0p%.0f",dR*1000)]	);
        t->Branch(jetName+Form("QglAxis1_dR0p%.0f",dR*1000),"vector<float>",&qglAxis1_dR[Form("0p%.0f",dR*1000)]	);
        t->Branch(jetName+Form("QglGenMult_dR0p%.0f",dR*1000),"vector<float>",&qglGenMult_dR[Form("0p%.0f",dR*1000)]	);
        t->Branch(jetName+Form("QglGenPtD_dR0p%.0f",dR*1000),"vector<float>",&qglGenPtD_dR[Form("0p%.0f",dR*1000)]	);
        t->Branch(jetName+Form("QglGenAxis2_dR0p%.0f",dR*1000),"vector<float>",&qglGenAxis2_dR[Form("0p%.0f",dR*1000)]	);
        t->Branch(jetName+Form("QglGenAxis1_dR0p%.0f",dR*1000),"vector<float>",&qglGenAxis1_dR[Form("0p%.0f",dR*1000)]	);
    }
}
...
void BareJets::setBranchAddresses(TTree* t, std::string prefix)
{
    ...
    for(const auto& dR : dRToProduce)
    {
        BareFunctions::SetBranchAddress(t,jetName+Form("QglMult_dR0p%.0f",dR*1000) ,&qglMult_dR[Form("0p%.0f",dR*1000)]	    );
        BareFunctions::SetBranchAddress(t,jetName+Form("QglPtD_dR0p%.0f",dR*1000)  ,&qglPtD_dR[Form("0p%.0f",dR*1000)]	    );
        BareFunctions::SetBranchAddress(t,jetName+Form("QglAxis2_dR0p%.0f",dR*1000),&qglAxis2_dR[Form("0p%.0f",dR*1000)]	);
        BareFunctions::SetBranchAddress(t,jetName+Form("QglAxis1_dR0p%.0f",dR*1000),&qglAxis1_dR[Form("0p%.0f",dR*1000)]	);
        BareFunctions::SetBranchAddress(t,jetName+Form("QglGenMult_dR0p%.0f",dR*1000),&qglGenMult_dR[Form("0p%.0f",dR*1000)]	);
        BareFunctions::SetBranchAddress(t,jetName+Form("QglGenPtD_dR0p%.0f",dR*1000),&qglGenPtD_dR[Form("0p%.0f",dR*1000)]	);
        BareFunctions::SetBranchAddress(t,jetName+Form("QglGenAxis2_dR0p%.0f",dR*1000),&qglGenAxis2_dR[Form("0p%.0f",dR*1000)]	);
        BareFunctions::SetBranchAddress(t,jetName+Form("QglGenAxis1_dR0p%.0f",dR*1000),&qglGenAxis1_dR[Form("0p%.0f",dR*1000)]	);
    }
